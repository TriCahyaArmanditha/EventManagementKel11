...
public class AdminRegistrationParticipant extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.admin_registration_participant);

        etName = findViewById(R.id.adminetName);
                    ...
                    databaseReference.child(participantId).setValue(participant)
                            .addOnCompleteListener(task -> {
                                if (task.isSuccessful()) {
                                    Toast.makeText(AdminRegistrationParticipant.this, "Pendaftaran berhasil!", Toast.LENGTH_SHORT).show();

                                    // Berpindah ke halaman RegistrationConfirmationActivity
                                    Intent intent = new Intent(AdminRegistrationParticipant.this, AdminConfirmation.class);
                                    intent.putExtra("participant_name", name);  // Mengirim nama peserta
                                    startActivity(intent);
                                    finish();
                                } else {
                                    Toast.makeText(AdminRegistrationParticipant.this, "Pendaftaran gagal.", Toast.LENGTH_SHORT).show();
                                }
                            });
                }
            } else {
                Toast.makeText(AdminRegistrationParticipant.this, "Silakan isi semua field.", Toast.LENGTH_SHORT).show();
            }
        });
    }
}
